...
// Scale to fit in unit cylinder, then shift back to original z range.
coords = zscale3(0.7) * ((coords - (0,0,0.5)) / sqrt(2)) + (0,0,0.5);

pen[] cols={
  rgb(0.9, 0.1, 0.1),   // (-1,-1,0) SW: fire red      } z=0: warm fire wheel,
  rgb(1.0, 0.45,0.0),   // ( 1,-1,0) SE: orange        }   hues CW around circle
  ...
  rgb(0.05,0.2, 0.9),   // ( 1,-1,1) SE: cobalt blue   }   hues CCW (reversed!)
