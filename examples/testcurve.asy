...
NURBCurveData conversion_4DBSpline_to_3DNurbs(BSpline BSpline_4D){
   NURBCurveData new_nurb;
...
BSpline conversion_3DNurbs_to_4DBSpline(NURBCurveData NURBS_3D){
   BSpline new_BSpline_4D;
   real[][] x = new_BSpline_4D.controlPoints;
   for(int j=0;j<NURBS_3D.controlPoints.length;++j){
      triple control_point_3D = NURBS_3D.controlPoints[j];
      real weight = NURBS_3D.weights[j];
...
triple[] conversion_RBezier_to_NRBezier(triple[] RBcontrolPoints, triple[] adjust_controlPoints, triple[] sample_points){
   ...
   for(int i = 0; i < n; ++ i){
      ...
      if (ad_vector_length > allowed_error){
   ...
   return conversion_RBezier_to_NRBezier(RBcontrolPoints,local_control_points, local_sample_points);
}   
...
void drawNURB(triple[] control_points, real[] knots, real[] weights){
   NURBCurveData original_nurb = NURBCurveData(control_points, knots, weights);
   BSpline BSpline_4D = conversion_3DNurbs_to_4DBSpline(original_nurb);
   int BSpline_degree = BSpline_4D.degree;
   int output_degree = 3;
   write("BSpline control points");
   write(control_points);
   ...
   write("BSpline control points.");
   write(BSpline_4D.controlPoints);

   NURBCurveData nurb_3D = conversion_4DBSpline_to_3DNurbs(BSpline_4D); //This nurb curve is composed of several Bezier segments
   ...
   while(Bezier_last_cp_index < n){
      ...
      if(NR_bool && Bezier_last_cp_index < n){
         draw(current_Bezier_cps[0]..controls current_Bezier_cps[1] and current_Bezier_cps[2]..current_Bezier_cps[3]);
      }
      else{
         for(int i = 0; i <= m; ++i){
            sample_points[i] = RBezier_evaluation(i/m, current_Bezier_cps, current_Bezier_weights);
         }
         triple[] NR_Bezier_control_points = conversion_RBezier_to_NRBezier(current_Bezier_cps, current_Bezier_cps, sample_points);
         
         if(NR_Bezier_control_points.length == 4){
            for(int i = 0; i < 4; ++i){
               dot(NR_Bezier_control_points[i],red);
               /*string cordx = string(NR_Bezier_control_points[i].x,2);
               string cordy = string(NR_Bezier_control_points[i].y,2);
               string cordz = string(NR_Bezier_control_points[i].z,2);
               label(cordx+","+cordy+","+cordz, NR_Bezier_control_points[i]);*/
            }
            draw(NR_Bezier_control_points[0]..controls NR_Bezier_control_points[1] and NR_Bezier_control_points[2]..NR_Bezier_control_points[3]);
         }
      }
...
//weights[5] = 6;
drawNURB(testPoints, testKnots, weights);