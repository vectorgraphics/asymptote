...
struct revolution {
  ...
  surface surface(int n=nslice, pen color(triple)) {
	...
	triple v2=vertex(i+1,j+step);
	triple v3=vertex(i,j+step);
	s.s[++m]=patch(v0--v1--v2--v3--cycle,
		       new pen[] {color(v0),color(v1),color(v2),color(v3)});
      }
    }
    return s;
  }

  void fill(picture pic=currentpicture, int n=nslice, pen color(triple)) {
    draw(pic,surface(n,color),color(c));
  }
  
  typedef pen colors(triple v);
  
  colors colors(pen p) {
    return prc() ? new pen(triple) {return p;} :
    new pen(triple v) {return currentlight.intensity(v-c)*p;};
  }

  surface surface(int n=nslice, pen p=currentpen) {
    return surface(n,colors(p));
  }
  
  void fill(picture pic=currentpicture, int n=nslice, pen p=currentpen) {
    fill(pic,n,colors(p));
  }
  ...
  void filldraw(picture pic=currentpicture, int n=nslice,
