...
//private routine that takes a triangle, checks if the line passes through 
//it, and draws the required line through the triangle
private void checktriangle (picture pic, pair[] pts, real[] vls, pen p)
{
  if(vls[0] < 0){
    if(vls[1] < 0){
      ...
      else case3(pic,new pair[]{pts[0],pts[2],pts[1]}, new real[]{vls[0],
		vls[2],vls[1]},p); //case3
    }
    else if(vls[1] == 0){
      if(vls[2] < 0) return;       //nothing to do
      else if(vls[2] == 0) case1(pic,pts[1], pts[2],p); //case1
      else case2(pic,new pair[]{pts[1], pts[0], pts[2]}, 
		new real[]{vls[1],vls[0],vls[2]},p); //case2
    }
    else {
      if(vls[2] < 0) case3(pic,new pair[]{pts[0],pts[1],pts[2]}, 
		new real[]{vls[0],vls[1],vls[2]},p); //case3
      else if(vls[2] == 0) case2(pic,new pair[]{pts[2],pts[0],pts[1]}, 
		new real[]{vls[2],vls[0],vls[1]},p); //case2
      else case3(pic,new pair[]{pts[2],pts[0],pts[1]}, 
		new real[]{vls[2],vls[0],vls[1]},p); //case3
    } 
  }
  else if(vls[0] == 0){
    if(vls[1] < 0){
      if(vls[2] < 0) return; //nothing to do
      else if(vls[2] == 0) case1(pic,pts[0], pts[2],p); //case1
      else case2(pic,new pair[]{pts[0], pts[1], pts[2]}, 
		new real[]{vls[0],vls[1],vls[2]},p); //case2
    }
    else if(vls[1] == 0){
      if(vls[2] < 0) case1(pic,pts[0],pts[1],p); //case1
      else if(vls[2] == 0) return; //special case - use finer partitioning.      
      else case1(pic,pts[0],pts[1],p); //case1
    }
    else {
    ...
    if(vls[1] < 0){
      ...
      if(vls[2] < 0) case3(pic,new pair[]{pts[0],pts[2],pts[1]}, 
		new real[]{vls[0],vls[2],vls[1]},p); //case3
      else if(vls[2] == 0) return; //nothing to do
      else return; //nothing to do
    } 
  }      
...
{    
  //check if arrays are of same size (should we be forgiving take smallest?):
  if (cl.length != p.length) 
    abort("contour: give arrays of same lengthfor contour levels and pens");
 
  //check if boundaries are good
  if( (x0 == x1) || (y0 == y1) ) 
    abort("bad area of contouring - give distinct points on each axis");
