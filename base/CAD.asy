struct sCAD
{
    ...
    pen
        ...
        // C
        pC,
        pFreehand,                // Begrenzung abgebrochener oder unterbrochener Schnitte,
                                // wenn die Begrenzung keine Mittellinie ist
        // E
        pE,
    ...
    {
        if ( bSmallBound )
            return scale(GetMeasurementBoundSize(bSmallBound = bSmallBound)) * unitcircle;
        else
            return (0,0) --
                   (-cos(radians(7.5)), -sin(radians(7.5))) * GetMeasurementBoundSize(bSmallBound = bSmallBound) --
                   (-cos(radians(7.5)), sin(radians(7.5))) * GetMeasurementBoundSize(bSmallBound = bSmallBound) --
                   cycle;
    }
                        ...
                        bool bSmallBound = false)
    {
        ...
        guide    gArrow = GetMeasurementBound(bSmallBound = bSmallBound);
        picture    picL;
        label(picL, L);
    ...
    {
        pair    pDiff = pTo - pFrom;
        pair    pPerpendicular = dblRelDistortion * linewidth(pFreehand) * unit(rotate(90) * pDiff);

        int        nNumOfSubDivisions = ceil(length(pDiff) / (dblRelDivisionLength * linewidth(pFreehand)));

        restricted real[]    dblDistortion = {1, -.5, .75, -.25, .25, -1, .5, -.75, .25, -.25};
        int                    nDistortion = 0;
