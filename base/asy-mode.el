...
;; Last modification: 11 June 2007 (Philippe Ivaldi)
;;
...
This varaible is not used when running the Windows OS.
See `asy-open-file'."
  :type 'file
  :group 'asymptote)
...
(defun asy-internal-shell (command &optional pass)
  "Execute 'command' in a inferior shell discarding outpout and
redirecting sdterr in the file given by the command `asy-log-filename'.
...
auto-clode non-nil means automatically close the compilation buffer.
stderr non-nil means redirect the standart output error to the file
...
In unix-like system the varaibles `ps-view-command' and `pdf-view-command' are used.
