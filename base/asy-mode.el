...
;; Last modification: 09 June 2007 (Philippe Ivaldi and John Bowman)
;;
  ...
  (if (and (boundp two-mode-bool) two-mode-bool)
    ...
    (progn
      (let*
		...
		(progn
		  (if (file-exists-p (asy-log-filename))
		      (delete-file (asy-log-filename)))
		  (concat asy-command-location
			  asy-command
			  " "
			  buffer-base-name
			  ".asy > " (asy-protect-file-name (asy-log-filename))))
    ...
    (insert-file-contents
     (if running-unix-p Filename
       (replace-regexp-in-string
        "//" ":/"
        (replace-regexp-in-string "/cygdrive/" "" Filename))))
    (beginning-of-buffer)
    ...
    (progn
      ...
      (save-excursion
        (let ((Filename (asy-get-temp-file-name))
	      ...
	      ((asy-compile-command
	       (concat asy-command-location
		       asy-command " -wait " (asy-protect-file-name Filename)
		       " > " (asy-protect-file-name (asy-log-filename)))))
	    (if (file-exists-p (asy-log-filename)) 
		(delete-file (asy-log-filename)))
...
(defvar asy-compilation-auto-close nil
  "Variable to comunicate with `asy-compilation-finish-function'.
...
(defun asy-internal-compile (command &optional pass auto-close)
  (setq asy-last-compilation-code -1)
  (let* ((compilation-buffer-name-function
          (lambda (mj) "*asy-compilation*")))
    (if (eq asy-compilation-buffer 'never)
        (let* ((log-p (string-match "> *\"\\(.*\.log\\)\"$" command))
               (log-file
                (if log-p
                    (match-string 1 command) (asy-log-filename)))
               (log-file-pro (asy-protect-file-name log-file))
               (command-tmp (if log-p
                                command (concat command " > " log-file-pro))))
          (write-region "" 0 log-file nil)
          (message command-tmp)
          (with-temp-buffer
            (shell-command command-tmp t)
            (beginning-of-buffer)
            (if (search-forward "! LaTeX Error" (point-max) t)
                (setq asy-last-compilation-code 1)
              (setq asy-last-compilation-code
                    (with-temp-buffer
      ...
      ((b-b-n  (if Filename Filename (file-name-sans-extension buffer-file-name)))
       ...
       (dirstderr (if (or (eq asy-compilation-buffer 'never) lasy-compile-tex)
                      (progn
                        ...
                        (concat " > " (asy-protect-file-name (asy-log-filename)))) "")))
    (if (or (file-newer-than-file-p
             (concat b-b-n ".tex")
             (concat b-b-n ".ps"))
            Force)
        (progn
          (asy-internal-compile (concat  "latex -halt-on-error " b-b-n-tex))
          (when (and (zerop asy-last-compilation-code) (file-readable-p (concat b-b-n ".asy")))
            ...
            (when (zerop asy-last-compilation-code)
              (asy-internal-compile (concat "latex -halt-on-error " b-b-n-tex))))
          (when (zerop asy-last-compilation-code)
            (asy-internal-compile (concat "dvips -q " b-b-n-dvi " -o " b-b-n-ps) nil t)
            (when (zerop asy-last-compilation-code)
...
If optional argument Force is t then force compilation."
  ...
  (let*
    ...
    (if (or (file-newer-than-file-p
            ...
            Force)
        (progn
          (asy-internal-compile (concat  "pdflatex -halt-on-error " b-b-n-tex))
          (when (and (zerop asy-last-compilation-code) (file-readable-p (concat b-b-n ".asy")))
            (asy-internal-compile (concat asy-command-location lasy-command " " b-b-n-asy dirstderr))
            (when (zerop asy-last-compilation-code)
              (asy-internal-compile (concat "pdflatex -halt-on-error " b-b-n-tex) t)))
          (when (zerop asy-last-compilation-code)
        ...
        (progn
          ...
          (when (and (zerop asy-last-compilation-code) (file-readable-p (concat b-b-n ".asy")))
            ...
            (when (zerop asy-last-compilation-code)
              (asy-internal-compile (concat "latex -halt-on-error " b-b-n-tex))))
          (when (zerop asy-last-compilation-code)
            (asy-internal-compile (concat "dvips -Ppdf " b-b-n-dvi " -o " b-b-n-ps))
            (when (zerop asy-last-compilation-code)
              (asy-internal-compile (concat "ps2pdf14 -dPDFSETTINGS=/prepress -dAutoFilterGrayImages=false -dAutoFilterColorImages=false -dColorImageFilter=/FlateEncode -dGrayImageFilter=/FlateEncode -dAutoRotatePages=/None " b-b-n-ps " " b-b-n-pdf) t)
              (when (zerop asy-last-compilation-code)
                (asy-open-file (concat b-b-n ".pdf"))))))
      (asy-open-file (concat b-b-n ".pdf")))))
