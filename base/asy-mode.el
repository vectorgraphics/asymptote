...
- GNU Emacs 22.0.50.1
* WindowsXP
- GNU Emacs 22.0.990.1 (i386-mingw-nt5.1.2600) of 2007-05-20 on NEUTRINO
  ...
  (if (and (boundp two-mode-bool) two-mode-bool)
    ...
    (progn
      (let*
        ...
        (asy-internal-compile asy-compile-command t)
...
(defun asy-compilation-wait(&optional auto-close)
  (let* ((buff (get-buffer "*asy-compilation*"))
    ...
    (when (and auto-close (zerop asy-last-compilation-code))
      (let ((buf (get-buffer "*asy-compilation*")))
        (delete-windows-on buf)
        (when (eq asy-compilation-buffer 'none)
          (kill-buffer buf))))))
  ...
  (let* ((compilation-buffer-name-function
    ...
    (if (eq asy-compilation-buffer 'never)
        (progn
          ...
          (shell-command (concat command " > asy.log")))
      (progn
  ...
  (let ((command
         (cond
          ((eq window-system 'x)
           ...
           (t  (error "Can Not Open This File.")))
          ))
  ...
  (let*
      ((b-b-n  (if Filename Filename (file-name-sans-extension buffer-file-name)))
       ...
       (dirstderr (if (or (eq asy-compilation-buffer 'never) lasy-compile-tex)
                      (progn
                        (write-region "" 0 (asy-log-filename) nil)
                        (concat " 2> " (asy-protect-file-name (asy-log-filename)))) "")))
...
If optional argument Force is t then force compilation."
  ...
  (let*
    ...
    (if (or (file-newer-than-file-p
            ...
            Force)
        (progn
          (asy-internal-compile (concat  "pdflatex -interaction=nonstopmode " b-b-n-tex))
          (when (and (zerop asy-last-compilation-code) (file-readable-p (concat b-b-n ".asy")))
            (asy-internal-compile (concat asy-command-location lasy-command " " b-b-n-asy dirstderr))
            (when (zerop asy-last-compilation-code)
              (asy-internal-compile (concat "pdflatex -interaction=nonstopmode " b-b-n-tex) t)))
          (when (zerop asy-last-compilation-code)
