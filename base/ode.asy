...
RKTableau EulerTableau = RKTableau(1,new real[] {0.0},
                                   new real[][] {{}},
                                   new real[][] {{1.0}});

RKTableau RK3Tableau = RKTableau(3,new real[] {0,0.5,1},
                                 new real[][] {{0.5},{-1,2}},
                                 new real[][] {{1/6,2/3,1/6}});

RKTableau RK4Tableau = RKTableau(4,new real[] {0,0.5,0.5,1},
                                 new real[][] {{0.5},{0,0.5},{0,0,1}},
                                 new real[][] {{1/6,1/3,1/3,1/6}});

RKTableau RKCK45Tableau = RKTableau(5,new real[] {0,1/5,3/10,3/5,1,7/8},
                            new real[][] {{1.5},
                                          {3/40,9/40},
                                          {3/10,-9/10,6/5},
                                          {-11/54,5/2,-70/27,35/27},
                                          {1631/55296,175/512,575/13824,44275/110592,253/4096}},
                            new real[][] {{2825/27648,0,18575/48384,13525/55296,277/14336,1/4}, // 4th order
                                          {37/378,0,250/621,125/594,0,512/1771}}); // 5th order

RKTableau RKF45Tableau = RKTableau(5,new real[] {0,1/4,3/8,12/13,1,1/2},
                            new real[][] {{1/4},
                                          {3/32,9/32},
...
RKTableau DP45Tableau = RKTableau(5,new real[] {0,1/5,3/10,4/5,8/9,1,1},
                           new real[][] {{1/5},
                                         {3/40,9/40},
                                         {44/45,-56/15,32/9},
                           ...
                           new real[][] {{35/384,0,500/1113,125/192,-2187/6784,11/84,0}, // 4th order?
              ...
              bool dynamic, real tolmin, real tolmax, RKTableau tableau)
{
  ...
  real[] predictions = {f(samplePositions[0],y)};
  for(int i=1; i < tableau.hSteps.length; ++i)
  {
    real yval = dot(predictions,tableau.weights[i-1]);
    predictions.push(f(samplePositions[i],y+h*yval));
  }
  ...
  if(dynamic)
  {
    ...
    real errorEstimate =
        abs(yIncrement[yIncrement.length-1][0]-yIncrement[yIncrement.length-2][0]);
...
real euler(real y, real f(real x, real y), real a, real b=a, int n=0,
     real h=0, bool dynamic=false, real tolmin=0, real tolmax=0)
{
  return RKbase(y,f,a,b,n,h,dynamic,tolmin,tolmax,EulerTableau);
}

real RK3(real y, real f(real x, real y), real a, real b=a, int n=0,
         real h=0, bool dynamic=false, real tolmin=0, real tolmax=0)
{
  return RKbase(y,f,a,b,n,h,dynamic,tolmin,tolmax,RK3Tableau);
}

real RK4(real y, real f(real x, real y), real a, real b=a, int n=0,
     real h=0, bool dynamic=false, real tolmin=0, real tolmax=0)
{
  return RKbase(y,f,a,b,n,h,dynamic,tolmin,tolmax,RK4Tableau);
}

real RKCK45(real y, real f(real x, real y), real a, real b=a, int n=0,
         real h=0, bool dynamic=false, real tolmin=0, real tolmax=0)
{
  return RKbase(y,f,a,b,n,h,dynamic,tolmin,tolmax,RKCK45Tableau);
}

real RKF45(real y, real f(real x, real y), real a, real b=a, int n=0,
            real h=0, bool dynamic=false, real tolmin=0, real tolmax=0)
{
  return RKbase(y,f,a,b,n,h,dynamic,tolmin,tolmax,RKF45Tableau);
}

real DP45(real y, real f(real x, real y), real a, real b=a, int n=0,
           real h=0, bool dynamic=false, real tolmin=0, real tolmax=0)
{
  return RKbase(y,f,a,b,n,h,dynamic,tolmin,tolmax,DP45Tableau);
}


// integrate a set of equations, f, from a to b in n steps using initial conditions y
real[] integrate(real[] y, real[] f(real,real[]), real a, real b, int n, RKTableau tableau)
{
  real[] y = copy(y);
  real h = (b-a)/n;
  real x = a;
...
{
  real sqrtRealEpsilon = sqrt(realEpsilon);

  real[] fx = f(x);

  real h = sqrtRealEpsilon*abs(fx[0]);
  if(h < sqrtRealEpsilon) h = sqrtRealEpsilon;
...
real[] newton(int iterations=100, real[] f(real[]), real[][] jacobian(real[]), real[] x)
{
  real[] x = copy(x);
...
real[] solveBVP(real[] f(real,real[]),
                real a,
                real b,
                real[] initial(real[]),
...
write(integrate(new real[]{4,-8},f,0,1,100,RK4Tableau));
write();
...
write(solveBVP(f,0.0,1.0,initial,discrepancy,guess=new real[]{-30},n=10,RK4Tableau,iterations=10));
write();
write(solveBVP(f,0.0,1.0,initial,discrepancy,guess=new real[]{-30},n=100,RK4Tableau,iterations=10));
write();
write(solveBVP(f,0.0,1.0,initial,discrepancy,guess=new real[]{-30},n=10000,RK4Tableau,iterations=10));
write();
*/
