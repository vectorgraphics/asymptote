...
// N.B.: Relative coordinates take the lower left corner of the block
// as (0, 0).
...
struct flowblock {
  ...
  // Returns the relative centre of the block.
  public pair f_centre();

  // Returns the absolute centre of the block.
  pair centre() {
    return shift(this.llcorner)*this.f_centre();
  }
  ...
  pair position(real x) {
    return shift(this.llcorner)*this.f_position(x);
  }
  ...
  public pair top() {
    return shift(this.llcorner)*this.f_top();
  } 
  public pair bottom() {
    return shift(this.llcorner)*this.f_bottom();
  } 
  public pair left() {
    return shift(this.llcorner)*this.f_left();
  } 
  public pair right() {
    return shift(this.llcorner)*this.f_right();
  } 
  public pair topleft() {
    return shift(this.llcorner)*this.f_topleft();
  } 
  public pair topright() {
    return shift(this.llcorner)*this.f_topright();
  } 
  public pair bottomleft() {
    return shift(this.llcorner)*this.f_bottomleft();
  } 
  public pair bottomright() {
    return shift(this.llcorner)*this.f_bottom();
  } 
  
  // Centres the block on the given coordinate.
  public void centre(pair loc) {
      this.llcorner = loc - this.f_centre(); 
  } 
...
flowblock flowrectangle(picture header, picture body, 
                        pen header_colour = gray(.8),
                        pen body_colour = currentpen, 
                        pair centre = (0,0), real dx = 3) 
{
...
flowblock flowrectangle(picture body, pen body_colour = currentpen, 
                        pair centre = (0,0), real dx = 3) 
{
  pair bound;
  path shape;
  ...
  block.draw = new picture (pen p) {
    ...
    draw(block, (0,0)--(0,bound.y+2*dx)--
                (bound.x+2*dx,bound.y+2*dx)--
                (bound.x+2*dx,0)--cycle, body_colour);
...
flowblock flowdiamond(picture pic, pair centre=(0,0), 
                      real ds = 5,
                      real dw = 1,
                      real height = 20,
                      real dh = 0) {
...
flowblock flowcircle(picture pic, pair centre=(0,0), real dr = 0) {
  real r;
  path shape;
  pair bound;
...
flowblock flowroundrectangle(picture pic, pair centre=(0,0),
                             real ds = 5, real dw = 0) {
  real a, b;
  path shape;
  pair bound;
...
flowblock flowbevel(picture pic, pair centre=(0,0), real dh = 5, real dw = 5) {
  real a, b;
  path shape;
  pair bound;
...
void drawflow(picture pic = currentpicture, path flow, 
  ...
  frame f = text.fit();
  pair centre = (min(f)+max(f))/2,
       textloc = ((1.2align.x-1)*centre.x,(1.2align.y-1)*centre.y)
                  +point(flow,pos*length(flow));
  draw(pic, flow, p, Arrow);
  add(pic, shift(textloc)*text);
}

picture text_to_pic(string text, real dy=0.2cm, 
