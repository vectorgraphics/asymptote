...
ASY_MODULE(phase2, m) {
    ...
    m.def("apply_int", [](ay::callable<int(int)> f, int x) {
        return f(x);
    });

    // Pass a string through a `string(string)` callback.
    m.def("apply_str", [](ay::callable<std::string(std::string)> f,
                          std::string s) {
        return f(s);
    });

    // Sum a callback applied to 0..n-1.
    m.def("sum_apply", [](ay::callable<int(int)> f, int n) {
        int total = 0;
    ...
    m.def("safe_divide", [](int a, int b) {
        if (b == 0) return ay::result<int>(false, 0);
        return ay::result<int>(true, a / b);
    });

    // String lookup: returns (found, value) for a fixed table.
    m.def("lookup_name", [](int id) {
        switch (id) {
            case 1: return ay::result<std::string>(true, "alice");
            case 2: return ay::result<std::string>(true, "bob");
            default: return ay::result<std::string>(false, "");
        }
    });

    // ---- callable + result combined -----------------------------
    // Find first int in [0, n) where pred(i) is true.
    m.def("find_first", [](ay::callable<bool(int)> pred, int n) {
        for (int i = 0; i < n; ++i) {
            if (pred(i)) return ay::result<int>(true, i);
        }
        return ay::result<int>(false, 0);
    });
}
