...
list(APPEND ASY_STATIC_LIBRARIES ZLIB::ZLIB)
...
if (WIN32)
    ...
    list(APPEND ASY_STATIC_LIBRARIES unofficial::getopt-win32::getopt)
endif()

# glm; mandatory for all builds
find_package(glm CONFIG)
if (glm_FOUND)
    list(APPEND ASY_STATIC_LIBRARIES glm::glm)
...
# curses
    if (UNIX)
        ...
        if (readline_FOUND)
            list(APPEND ASY_STATIC_LIBRARIES PkgConfig::readline)
    ...
    elseif(WIN32)
        ...
        if (unofficial-readline-win32_FOUND)
            list(APPEND ASY_STATIC_LIBRARIES unofficial::readline-win32::readline)
            list(APPEND ASY_MACROS HAVE_LIBREADLINE)
        else ()
            message(FATAL_ERROR "readline not found; will compile without libreadline")
        endif()
...
if (ENABLE_CURL)
    find_package(CURL)
    if (CURL_FOUND)
        list(APPEND ASY_STATIC_LIBRARIES CURL::libcurl)
...
if (ENABLE_THREADING)
    if (UNIX)
        ...
        if(CMAKE_USE_PTHREADS_INIT)
            list(APPEND ASY_STATIC_LIBRARIES Threads::Threads)
    ...
    elseif(WIN32)
        ...
        if(PThreads4W_FOUND)
            list(APPEND ASY_STATIC_LIBRARIES PThreads4W::PThreads4W)
            list(APPEND ASY_MACROS HAVE_PTHREAD=1)
        else()
            message(FATAL_ERROR "No thread library specified")
        endif()
...
if (ENABLE_GSL)
    find_package(GSL)
    if (GSL_FOUND)
        list(APPEND ASY_STATIC_LIBRARIES GSL::gsl)
...
find_package(Eigen3 CONFIG)
    if (Eigen3_FOUND)
        list(APPEND ASY_STATIC_LIBRARIES Eigen3::Eigen)
...
# OpenGL (fallback renderer when Vulkan is unavailable at runtime)
if (ENABLE_GL)
    find_package(OpenGL)
    if (OPENGL_FOUND)
        ...
        message(STATUS "OpenGL not found; will compile without OpenGL fallback support")
    endif()

    find_package(GLUT)
    if (GLUT_FOUND)
        list(APPEND ASY_STATIC_LIBRARIES GLUT::GLUT)
        list(APPEND ASY_MACROS HAVE_LIBGLUT)
    else()
        message(STATUS "GLUT not found; will compile without OpenGL fallback support")
    endif()

    if (OPENGL_FOUND AND GLUT_FOUND)
        # Include bundled GLEW (static, single-file build)
        list(APPEND ASYMPTOTE_INCLUDES ${ASY_SRC_DIR}/backports/glew/include)
        list(APPEND ASY_MACROS GLEW_NO_GLU GLEW_BUILD)
        # glew.c is compiled as C, not C++
        set(GLEW_C_SOURCE ${ASY_SRC_DIR}/glew.c)
        set_source_files_properties(${GLEW_C_SOURCE} PROPERTIES LANGUAGE C)
        list(APPEND CORE_SOURCE_FILES ${GLEW_C_SOURCE})
    endif()
else()
    message(STATUS "Disabling OpenGL fallback support")
endif()
...
if (ENABLE_RPC_FEATURES)
    if(UNIX)
        ...
        list(APPEND ASY_STATIC_LIBRARIES PkgConfig::TIRPC)
    endif()
    ...
    if (WIN32)
        ...
        list(APPEND ASY_STATIC_LIBRARIES fmem)
...
if (ENABLE_FFTW3)
    ...
    if (FFTW3_USABLE)
        list(APPEND ASY_STATIC_LIBRARIES FFTW3::fftw3)
