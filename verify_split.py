...
def verify_full_coverage(name, covered_ranges, total_lines):
    ...
    for start, end in covered_ranges:
        ...
        if overlap:
            print(f"  FAIL: {name} has overlapping coverage at lines"
                  f" {sorted(overlap)[:10]}")
            return False
        covered |= span
    expected = set(range(1, total_lines + 1))
    if covered == expected:
        print(f"  OK: All {total_lines} lines of {name} are covered"
...
def main():
    ...
    ok &= check_line("errortest.asy[1] == '/*****'",
                      orig, 1, "/*****\n")
    ok &= check_line("errortest.asy[7] == ' *****/'",
                      orig, 7, " *****/\n")
    ok &= check_line("errortest.asy[8] is blank",
                      orig, 8, "\n")
    ...
    ok &= check_line("errortest.asy[9] == '// name.cc'",
                      orig, 9, "// name.cc\n")
    orig_ranges.append((9, 9))

    # names.asy lines 2-71 == errortest.asy lines 10-79
    ok &= check("names.asy[2:71] == errortest.asy[10:79]",
                 lines(names, 2, 71), lines(orig, 10, 79))
    orig_ranges.append((10, 79))

    # Line 80: blank separator.
    ok &= check_line("errortest.asy[80] is blank",
                      orig, 80, "\n")
    ...
    ok &= check("expressions.asy[3:111] == errortest.asy[81:189]",
                 lines(expressions, 3, 111), lines(orig, 81, 189))
    orig_ranges.append((81, 189))

    # Line 190: blank separator.
    ok &= check_line("errortest.asy[190] is blank",
                      orig, 190, "\n")
    orig_ranges.append((190, 190))

    # statements.asy lines 3-60 == errortest.asy lines 191-248
    ok &= check("statements.asy[3:60] == errortest.asy[191:248]",
                 lines(statements, 3, 60), lines(orig, 191, 248))
    orig_ranges.append((191, 248))

    # Line 249: blank separator.
    ok &= check_line("errortest.asy[249] is blank",
                      orig, 249, "\n")
    orig_ranges.append((249, 249))

    # environment.asy lines 3-52 == errortest.asy lines 250-299
    ok &= check("environment.asy[3:52] == errortest.asy[250:299]",
                 lines(environment, 3, 52), lines(orig, 250, 299))
    orig_ranges.append((250, 299))

    # Line 300: blank separator.
    ok &= check_line("errortest.asy[300] is blank",
                      orig, 300, "\n")
    ...
    ok &= check_line("errortest.asy[301] == '// Test permissions.'",
                      orig, 301, "// Test permissions.\n")
    orig_ranges.append((301, 301))

    # FIRST CHUNK: permissions.asy lines 2-115 == errortest.asy lines 302-415
    ok &= check("permissions.asy[2:115] == errortest.asy[302:415]",
                 lines(permissions, 2, 115), lines(orig, 302, 415))
    orig_ranges.append((302, 415))

    # Line 416: blank separator.
    ok &= check_line("errortest.asy[416] is blank",
                      orig, 416, "\n")
    ...
    ok &= check("var.asy[1:55] == errortest.asy[417:471]",
                 lines(var, 1, 55), lines(orig, 417, 471))
    orig_ranges.append((417, 471))

    # Line 472: blank separator.
    ok &= check_line("errortest.asy[472] is blank",
                      orig, 472, "\n")
    ...
    # to orig 473)
    ok &= check("keyword_rest.asy[1:42] == errortest.asy[473:514]",
                 lines(keyword_rest, 1, 42), lines(orig, 473, 514))
    orig_ranges.append((473, 514))

    # Line 515: blank separator.
    ok &= check_line("errortest.asy[515] is blank",
                      orig, 515, "\n")
    ...
    ok &= check_line("errortest.asy[516] == '// template import errors'",
                      orig, 516, "// template import errors\n")
    orig_ranges.append((516, 516))

    # FIRST CHUNK: templates.asy lines 2-29 == errortest.asy lines 517-544
    ok &= check("templates.asy[2:29] == errortest.asy[517:544]",
                 lines(templates, 2, 29), lines(orig, 517, 544))
    orig_ranges.append((517, 544))

    # Line 545: blank separator.
    ok &= check_line("errortest.asy[545] is blank",
                      orig, 545, "\n")
    ...
    # — different wording).
    ok &= check_line("errortest.asy[546] == '// autounravel errors'",
                      orig, 546, "// autounravel errors\n")
    orig_ranges.append((546, 546))

    # autounravel.asy lines 2-29 == errortest.asy lines 547-574
    ok &= check("autounravel.asy[2:29] == errortest.asy[547:574]",
                 lines(autounravel, 2, 29), lines(orig, 547, 574))
    orig_ranges.append((547, 574))

    # SECOND CHUNK of templates: templates.asy lines 30-59
    # == errortest.asy lines 575-604
    ok &= check("templates.asy[30:59] == errortest.asy[575:604]",
                 lines(templates, 30, 59), lines(orig, 575, 604))
    orig_ranges.append((575, 604))

    # SECOND CHUNK of permissions: permissions.asy lines 116-151
    # == errortest.asy lines 605-640
    ok &= check("permissions.asy[116:151] == errortest.asy[605:640]",
                 lines(permissions, 116, 151), lines(orig, 605, 640))
    ...
    ok &= check("operators.asy[2:49] == errortest.asy[641:688]",
                 lines(operators, 2, 49), lines(orig, 641, 688))
    orig_ranges.append((641, 688))

    # Line 689: "}" without trailing newline.  operators.asy line 50
    # is "}\n" — the split file adds a trailing newline.
    ok &= check_line("errortest.asy[689] == '}'",
                      orig, 689, "}")
    ...
    ok &= check("keyword_rest.errors[1:40] == errors[1:40]",
    ...
    ok &= check("names.errors[1:17] == errors[41:57]",
    ...
    ok &= check("expressions.errors[1:19] == errors[58:76]",
    ...
    ok &= check("statements.errors[1:9] == errors[77:85]",
    ...
    ok &= check("environment.errors[1:2] == errors[86:87]",
    ...
    ok &= check("permissions.errors[1:45] == errors[88:132]",
    ...
    ok &= check("var.errors[1:24] == errors[133:156]",
    ...
    ok &= check("templates.errors[1:16] == errors[157:172]",
    ...
    ok &= check("autounravel.errors[1:8] == errors[173:180]",
    ...
    ok &= check("templates.errors[17:32] == errors[181:196]",
    ...
    ok &= check("permissions.errors[46:54] == errors[197:205]",
    ...
    ok &= check("operators.errors[1:9] == errors[206:214]",
