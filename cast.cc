...
{
  // NOTE: I am making error messages equal so that the trans and
  // transHeader of function definitions with error in the parameters
  // will work.
  // For pragmatic reasons in resolving functions, assume errors are
  // distinct.
  ...
  // Equivalence of overloaded types is based on a match of subtypes.
  if (t1->kind == ty_overloaded) {
    overloaded *set = (overloaded *)t1;
  ...
  // Array equivalence is based purely on the cells.
  if (t1->kind == ty_array && t2->kind == ty_array) {
    return equivalent(((array *)t1)->celltype,((array *)t2)->celltype);
  }

  // Structures must be based on the same definition.  Hence, pointer
  // equality is necessary.
  if (t1->kind == ty_record)
    return false;

  // Functions are equivalent if their results and parameters are.
  ...
  // As the remaining types are primitive, they can are compared using
  // the kind field.
