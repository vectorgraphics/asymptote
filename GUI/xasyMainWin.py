...
class xasyMainWin:
    def __init__(self, master, file=None, magnification=1.0):
        ...
        site = ""
        if not PILAvailable:
            messagebox.showerror("Failed Dependencies",
                                 "An error occurred loading the required PIL library. Please install Pillow from http://pypi.python.org/pypi/Pillow")
            self.parent.destroy()
            sys.exit(1)
        if file != None:
            self.loadFile(file)
    ...
    def createWidgets(self):
        ...
        self.toolVertiMoveButton = Button(self.toolBar, command=self.toolVertiMoveCmd,
                                          image=self.toolIcons["vertiMove"])
        self.toolVertiMoveButton.grid(row=3, column=0, sticky=N + S + E + W)
        self.toolHorizMoveButton = Button(self.toolBar, command=self.toolHorizMoveCmd,
                                          image=self.toolIcons["horizMove"])
        ...
        self.toolDrawLinesButton = Button(self.toolBar, command=self.toolDrawLinesCmd,
                                          image=self.toolIcons["drawLines"])
        ...
        self.toolDrawEllipButton = Button(self.toolBar, command=self.toolDrawEllipCmd,
                                          image=self.toolIcons["drawEllip"], state=DISABLED, relief=FLAT)
        # self.toolDrawEllipButton.grid(row=7,column=0,sticky=N+S+E+W)
        self.toolFillEllipButton = Button(self.toolBar, command=self.toolFillEllipCmd,
                                          image=self.toolIcons["fillEllip"], state=DISABLED, relief=FLAT)
        # self.toolFillEllipButton.grid(row=7,column=1,sticky=N+S+E+W)
        self.toolDrawShapeButton = Button(self.toolBar, command=self.toolDrawShapeCmd,
                                          image=self.toolIcons["drawShape"])
        self.toolDrawShapeButton.grid(row=8, column=0, sticky=N + S + E + W)
        self.toolFillShapeButton = Button(self.toolBar, command=self.toolFillShapeCmd,
                                          image=self.toolIcons["fillShape"])
    ...
    def applyOptions(self):
        ...
        if not quickAsyRunning():
            if messagebox.askyesno("Xasy Error",
                                   "Asymptote could not be executed.\r\nTry to find Asymptote automatically?"):
    ...
    def loadFile(self, name):
        ...
        try:
            ...
            if self.autoMakeScript or messagebox.askyesno("Error Opening File",
    ...
    def fileOpenCmd(self):
        ...
        if self.undoRedoStack.changesMade():
            ...
            if str(result) == messagebox.CANCEL:
                ...
                self.fileSaveCmd()
        filename = filedialog.askopenfilename(filetypes=[("asy files", "*.asy"), ("All files", "*")], title="Open File",
                                              parent=self.parent)
    ...
    def fileSaveCmd(self):
        ...
        self.releaseLock()
        if self.filename == None:
            filename = filedialog.asksaveasfilename(defaultextension=".asy", filetypes=[("asy files", "*.asy")],
                                                    initialfile="newDrawing.asy", parent=self.parent, title="Save File")
            if type(filename) != type((0,)) and filename != None and filename != '':
                self.filename = filename
        if self.filename != None:
    ...
    def exportFile(self, inFile, outFormat):
        ...
        if inFile == None:
            ...
            choice = messagebox._show("xasy",
                                      "File has been modified.\nOnly saved changes can be exported.\nDo you want to save changes?",
                                      icon=messagebox.QUESTION, type=messagebox.YESNOCANCEL)
            choice = str(choice)
        ...
        outfilename = filedialog.asksaveasfilename(defaultextension='.' + outFormat,
                                                   filetypes=[(outFormat + " files", "*." + outFormat)],
                                                   initialfile=name, parent=self.parent, title="Choose output file")
    ...
    def updateSelectedButton(self, newB):
        ...
        if self.editor != None:
            ...
            if self.editor.modified:
                self.undoRedoStack.add(editDrawnItemAction(self, self.itemBeingEdited, copy.deepcopy(self.editor.shape),
                                                           self.fileItems.index(self.editor.shape)))
        if newB not in (self.toolSelectButton, self.toolMoveButton, self.toolHorizMoveButton, self.toolVertiMoveButton,
                        self.toolRotateButton):
            self.clearSelection()
    ...
    def translateSomething(self, ID, translation, specificItem=None, specificIndex=None):
        ...
        if isinstance(item, xasyText) or isinstance(item, xasyScript):
            ...
            item.imageList[index].originalImage.bbox = bbox[0] + translation[0], bbox[1] + translation[1], bbox[2] + \
                                                       translation[0], bbox[3] + translation[1]
        else:
    ...
    def rotateSomething(self, ID, theta, origin, specificItem=None, specificIndex=None):
        ...
        if isinstance(item, xasyText) or isinstance(item, xasyScript):
            ...
            item.imageList[index].image = item.imageList[index].originalImage.rotate(
                item.imageList[index].originalImage.theta * 180.0 / math.pi, expand=True, resample=Image.BICUBIC)
            ...
            newTopLeft = (
            min(p0[0], p1[0], p2[0], p3[0]), -max(p0[1], p1[1], p2[1], p3[1]))  # switch back to screen coords
    ...
    def itemEdit(self, item):
        ...
        if isinstance(item, xasyScript):
            ...
            theText = simpledialog.askstring(title="Xasy - Text", prompt="Enter text to display:",
                                             initialvalue=item.label.text, parent=self.parent)
    ...
    def itemMouseUp(self, event):
        ...
        if self.amDragging:
            ...
            self.undoRedoStack.add(
                translationAction(self, itemList, indexList, (self.distanceDragged[0], -self.distanceDragged[1])))
            self.amDragging = False
    ...
    def itemSelect(self, event):
        ...
        if self.selectedButton in [self.toolSelectButton, self.toolMoveButton, self.toolVertiMoveButton,
                                   self.toolHorizMoveButton, self.toolRotateButton]:
            self.freeMouseDown = False
        if self.selectedButton == self.toolSelectButton or (
                len(self.mainCanvas.find_withtag("selectedItem")) <= 1 and self.selectedButton in [self.toolMoveButton,
                                                                                                   self.toolVertiMoveButton,
                                                                                                   self.toolHorizMoveButton,
                                                                                                   self.toolRotateButton]):
            self.clearSelection()
    ...
    def canvMotion(self, event):
        self.coords.config(
            text="(%.3f,%.3f)" % (self.mainCanvas.canvasx(event.x) / self.magnification,
                                  -self.mainCanvas.canvasy(event.y) / self.magnification)
        )
    ...
    def startDraw(self, event):
        ...
        if self.selectedButton == self.toolDrawEllipButton:
            ...
            theText = simpledialog.askstring(title="Xasy - Text", prompt="Enter text to display:", initialvalue="",
                                             parent=self.parent)
            if theText != None and theText != "":
            ...
            self.releaseLock()
            self.updateSelectedButton(self.toolSelectButton)
        elif self.selectedButton in [self.toolDrawLinesButton, self.toolDrawBeziButton, self.toolDrawPolyButton,
                                     self.toolDrawShapeButton, self.toolFillPolyButton, self.toolFillShapeButton]:
            self.inDrawingMode = True
            try:
                if len(self.itemBeingDrawn.path.nodeSet) == 0:
                    raise Exception
                else:
                    if self.selectedButton in [self.toolDrawLinesButton, self.toolDrawPolyButton,
                ...
                if self.selectedButton == self.toolDrawLinesButton:
                    ...
                    path.initFromNodeList([(x, -y), (x, -y), 'cycle'], ['..', '..'])
                if self.selectedButton in [self.toolDrawLinesButton, self.toolDrawBeziButton, self.toolDrawPolyButton,
                                           self.toolDrawShapeButton]:
                    self.itemBeingDrawn = xasyShape(path, pen=asyPen(self.penColor, self.penWidth, self.penOptions))
                else:
                    if self.penOptions.find("fillrule") != -1 or self.penOptions.find(
                            "evenodd") != -1 or self.penOptions.find("zerowinding") != -1:
        ...
        if self.freeMouseDown and self.editor != None:
            ...
            if self.editor.modified:
                self.undoRedoStack.add(editDrawnItemAction(self, self.itemBeingEdited, copy.deepcopy(self.editor.shape),
                                                           self.fileItems.index(self.editor.shape)))
            self.editor = None
        elif self.selectedButton in (
        self.toolSelectButton, self.toolMoveButton, self.toolVertiMoveButton, self.toolHorizMoveButton,
        self.toolRotateButton):
    ...
    def canvLeftUp(self, event):
        ...
        if self.dragSelecting:
            self.hideSelectionBox()
            self.dragSelecting = False
            self.mainCanvas.addtag_enclosed("enclosed", self.selectDragStart[0], self.selectDragStart[1],
                                            self.mainCanvas.canvasx(event.x), self.mainCanvas.canvasy(event.y))
    ...
    def canvDrag(self, event):
        ...
        if self.selectedButton == self.toolSelectButton and self.editor == None:
            ...
            if bbox != None:
                p1 = self.selectDragStart[0] - self.selectBboxMidpoint[0], -self.selectDragStart[1] - \
                     self.selectBboxMidpoint[1]
                ...
                if not self.inRotatingMode:
                    ...
                    for ID in IDList:
                        ...
                        if item not in itemList:
                            ...
                            indexList[itemList.index(item)].append(self.findItemImageIndex(item, ID))
                    self.undoRedoStack.add(
                        rotationAction(self, itemList, indexList, self.currentRotationAngle, self.selectBboxMidpoint))
    ...
    def popupClearTransform(self):
        self.undoRedoStack.add(
            clearItemTransformsAction(self, self.itemPopupMenu.item, copy.deepcopy(self.itemPopupMenu.item.transform)))
        ...
        for ID in IDs:
            ...
            if not isinstance(item, xasyScript):
                ...
                if isinstance(item, xasyText):
                    ...
                    self.undoRedoStack.add(
                        editLabelPenAction(self, temp, asyPen(self.penColor, self.penWidth, self.penOptions),
                                           self.fileItems.index(item)))
                else:
                    temp = copy.deepcopy(item)
                    item.pen = asyPen(self.penColor, self.penWidth, self.penOptions)
                    item.drawOnCanvas(self.mainCanvas, self.magnification)
                    self.undoRedoStack.add(
                        editDrawnItemAction(self, temp, copy.deepcopy(item), self.fileItems.index(item)))
        if madeAChange:
            self.undoRedoStack.add(beginActionGroup)
