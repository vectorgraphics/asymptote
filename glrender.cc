...
void initShaders()
{
  ...
  if(vertex.empty() || count.empty() || fragment.empty() || blend.empty() || screen.empty())
    noShaders();
...
void refreshBuffers()
{
  ...
  if(fragments > maxFragments) {
    ...
    glBindBuffer(GL_SHADER_STORAGE_BUFFER,camp::colorBuffer);
