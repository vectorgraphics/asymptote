  ...
  void operator init(triple[] control_points,real[] knots,real[] weights) {
    ...
    while(Bezier_last_cp_index<n) {
      ...
      if(NR_bool&&Bezier_last_cp_index<n) {
        ...
        for(int i=0;i<m;++i) {
          data_points[i]=RBezier_curve_evaluation(i/(m-1),current_Bezier_cps,current_Bezier_weights);
          sample_points[i]=de_casteljau(i/(m-1),current_Bezier_cps);
        }
        real tolerance=NURBStolerance*norm(new triple[][] {current_Bezier_cps});
        triple[] NR_Bezier_control_points=conversion_RBezier_to_NRBezier(data_points,sample_points,sample_points,tolerance);
...
struct NURBSsurface{
    ...
    void operator init(triple[][] control_points,real[] U_knot,real[] V_knot,real[][] weights) {
          ...
          for(int j=0;j<ceilquotient(RBezier_3D.controlPoints[i].length,q+1);++j){
            ...
            for(int u=0;u<=sm*p;++u){
              for(int v=0;v<=sm*q;++v){
                data_points[u][v]=RBezier_surface_evaluation(u/p,v/q,matrix_trunc,weight_trunc);
                sample_points[u][v]=de_casteljau(u/p,v/q,matrix_trunc);
              }
            }
            extend_cp=conversion_RBezier_to_NRBezier(data_points,sample_points,sample_points,0.01);
            for(triple[] p:extend_cp)
              dot(p,pink);
            for(int u=0;u<=p;++u){
              for(int v=0;v<=q;++v){
        ...
        //for(triple[] p: RBezier_3D.controlPoints)
          //dot(p,black);
        triple[][] row_trunc;
